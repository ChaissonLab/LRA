...
RefineBtwnClusters_chain(int K, int W, vector<Primary_chain> &Primary_chains, vector<Cluster*> &RefinedClusters, vector<Cluster> &RevBtwnCluster, 
	...
	while (c < Primary_chains[p].chains[h].ch.size()) {
		...
		if (SpaceLength >= 100 and SpaceLength <= 100000 and RefinedClusters[cur]->chromIndex == RefinedClusters[prev]->chromIndex) {//used to be 100000; mapping contigs requires larger threshold;
			// btwnClusters have GenomePos, st, matches, coarse
			// This function also set the "coarse" flag for RefinedClusters[cur]
	...
	if (qe > qs and te > ts) {
		...
		if (SpaceLength >= 100 and  SpaceLength < 50000 and te+500 < genome.lengths[RefinedClusters[rh]->chromIndex]) { // used (1000, 6000)
	...
	if (qe > qs and te > ts) {
		SpaceLength = min(qe - qs, te - ts);
		if (SpaceLength >= 100 and SpaceLength < 50000 and te+500 < genome.lengths[RefinedClusters[lh]->chromIndex]) { // used (1000, 6000)
			GenomePos lrts=0, lrlength=0;
			if (st==0) { 
				if (ts>500) lrts=500;
				lrlength=lrts;	
