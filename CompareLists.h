...
// Compare minimizers from reference and reads with frequency `vector<tuple<tup, tup, T>> &result`
template<typename tup, typename Tup, typename T> 
void CompareLists(typename vector<tup>::iterator qBegin, typename vector<tup>::iterator qEnd, 
						...
						vector<tuple<tup, tup, T>> &result, vector<T> &mmfreqs, Options &opts, bool Global, int64_t maxDiagNum = 0,
				...
				for (GenomePos ti = tsStart; ti != tsi; ti++) {
					for (GenomePos qi = qsStart; qi <= qs; qi++) {
						if (maxDiagNum != 0 and minDiagNum != 0) {
							uint64_t Diag = (uint64_t) tBegin[ti].pos - (uint64_t) qBegin[qi].pos;
							if (Diag <= maxDiagNum and Diag >= minDiagNum) {
								result.push_back(tuple<tup, tup, T>(qBegin[qi], tBegin[ti], mmfreqs[ti]));
							}
						}
						else {
							result.push_back(tuple<tup, tup, T>(qBegin[qi], tBegin[ti], mmfreqs[ti]));
...
template<typename tup, typename Tup, typename T> 
void CompareLists(vector<tup> &query, vector<tup> &target, vector<tuple<tup, tup, T> > &result, vector<T> &mmfreqs, Options &opts, bool Global) {
	CompareLists<tup, Tup>(query.begin(), query.end(), target.begin(), target.end(), result, mmfreqs, opts, Global);
}
